...
func (app *App) findHandlerProcessForm(r *http.Request, accessLogDetails *carbonapipb.AccessLogDetails, logger *zap.Logger) (findForm, error) {
	var res findForm
...
func (app *App) findHandler(w http.ResponseWriter, r *http.Request) {
	...
	form, err := app.findHandlerProcessForm(r, &toLog, logger)
	toLog.Targets = []string{form.query}
	...
	query := form.query
	if form.format == completerFormat {
		query = getCompleterQuery(query)
	}

	if form.format == "" {
		form.format = treejsonFormat
	}
	...
	if form.useCache {
		tc := time.Now()
		response, err := app.findCache.Get(form.cacheKey)
		td := time.Since(tc).Nanoseconds()
		apiMetrics.FindCacheOverheadNS.Add(td)
	...
	default:
		err = errors.Errorf("Unknown format %s", form.format)
	}
